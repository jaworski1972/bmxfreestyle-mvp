...
const APP_URL = (process.env.APP_URL || "https://www.bmxseries.pl").replace(/\/$/, "");
...
function resendErrorMessage(error) {
  ...
  return parts.length ? parts.join(" ") : "Unknown Resend error.";
}

function parseResendSendResponse(response) {
  if (!response || typeof response !== "object") {
    throw new Error("Resend returned empty response.");
  }

  if (response.error) {
    throw new Error(`Resend email error: ${resendErrorMessage(response.error)}`);
  }

  const providerMessageId = response.data?.id || response.id || null;
  if (!providerMessageId) {
    throw new Error("Resend did not confirm email delivery.");
  }

  return { providerMessageId };
}

async function sendMail({ to, subject, html, client }) {
  const resend = client || resendClient();
  ...
  return parseResendSendResponse(response);
}
...
function registrationReceivedHtml({ event, category, registration }) {
  ...
  const confirmUrl = registration.confirmation_token ? confirmationUrl(registration.confirmation_token) : "";
  const currentStatusLabel = statusLabel(registration.status || "pending_review");
  const currentStatusExplanation = statusExplanation(registration.status || "pending_review");
  return `
      ...
      <div style="max-width:620px;margin:0 auto;padding:26px 14px;">
        <div style="background:#ffffff;border:1px solid #dde2e8;border-radius:18px;overflow:hidden;">
          ...
          <div style="padding:24px;">
            ...
            <p style="margin:0 0 20px;font-size:16px;line-height:1.55;">Status zgłoszenia: <strong>${escapeHtml(currentStatusLabel)}</strong>. ${escapeHtml(currentStatusExplanation)}</p>
            ${confirmUrl ? `<div style="border:1px solid #d7ff2f;border-radius:12px;background:#f7ffe8;padding:18px;margin:0 0 20px;">
              <p style="margin:0 0 10px;font-size:15px;line-height:1.5;"><strong>Potwierdzenie zgłoszenia i kod QR:</strong></p>
              <p style="margin:0;"><a href="${escapeHtml(confirmUrl)}" style="color:#0f1115;font-weight:900;word-break:break-word;">${escapeHtml(confirmUrl)}</a></p>
            </div>` : ""}
